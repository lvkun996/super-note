...
export function HelpDocumentation({ canvasPluginEnabled, shortcuts }: HelpDocumentationProps) {
  const operations = [
    ...
    ["保存内容", "当前标签有文件路径时直接保存；新内容会打开保存窗口，并优先使用设置中的默认文件保存位置。"],
    ["搜索内容", "点击右上角搜索图标或使用快捷键，可在文本模块和画板文字中定位匹配项。"],
    ["竖向选择", "在文本模块中按住鼠标左键片刻后竖向拖动，可连续选择多行内容；也支持中键直接拖动选择。"],
    ["关闭标签", "关闭最后一个标签后会进入空工作区。"],
    ["切换标签栏位置", "设置中可选择顶栏模式或侧边模式；标签支持拖拽排序，右键可置顶、删除、编辑名称或在资源管理器打开。"],
  ];
