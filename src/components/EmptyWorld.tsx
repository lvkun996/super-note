import { uiText } from "../../electron/uiLanguage";
import { EditOutlined, FileTextOutlined, PlusOutlined } from "@ant-design/icons";

export function EmptyWorld() {
  return (
    <div className="empty-world" aria-label={uiText("没有标签页")}>
      <div className="empty-world-glow empty-world-glow-one" aria-hidden="true" />
      <div className="empty-world-glow empty-world-glow-two" aria-hidden="true" />
      <div className="empty-world-content">
        <div className="empty-world-mark" aria-hidden="true">
          <EditOutlined />
        </div>
        <div className="empty-world-kicker">SUPER NOTE · YOUR QUIET SPACE</div>
        <h1 className="empty-world-title">{uiText("编写你的世界")}</h1>
        <p className="empty-world-description">{uiText("捕捉一闪而过的念头，把零散灵感慢慢写成自己的宇宙。")}</p>
        <div className="empty-world-hints" aria-label={uiText("快速开始提示")}>
          <span className="empty-world-hint">
            <PlusOutlined />
            <span>
              <strong>{uiText("新建文本")}</strong>
              <small>{uiText("点击标签栏的 +")}</small>
            </span>
          </span>
          <span className="empty-world-hint">
            <FileTextOutlined />
            <span>
              <strong>{uiText("打开文件")}</strong>
              <small>{uiText("直接拖入这个窗口")}</small>
            </span>
          </span>
        </div>
      </div>
    </div>
  );
}
